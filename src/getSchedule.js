...
const weekdaySpecies = (weekday) => species.filter((specie) =>
  specie.availability.includes(weekday)).map(({ name }) => name);
...
const theWeekdaySchedule = (weekday) => {
  ...
  return weekday === 'Monday'
    ...
    : { officeHour: `Open from ${open}am until ${close}pm`, exhibition: weekdaySpecies(weekday) };
};
const businessSchedule = (schedule, weekday) =>
  ({ ...schedule, [weekday]: theWeekdaySchedule(weekday) });
const showSchedule = () => theWeekdays().reduce(businessSchedule, {});
